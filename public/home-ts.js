...
let checkBoxinitial = false; /** variable for checking that checkbox is checked or not */
function checkBox() {
...
function powerSq(calc) {
    let num = Number(calc.display.value);
...
function trignoFunc(calc, value) {
    ...
    let num = Number(calc.display.value); /**stores user input */
    console.log("clicked");
    /**
...
function calculate(calc) {
    ...
    if (calc.display.value.includes("!")) {
        let size = calc.display.value.length;
        let n = Number(calc.display.value.substring(0, size - 1));
        let f = 1;
        for (let i = 2; i <= n; i++)
            f = f * i;
